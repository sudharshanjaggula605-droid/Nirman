...
export async function submitSupportRequestAction(
  ...
  try {
    ...
    const generatedReqNum = `NIR-${Date.now().toString().slice(-4)}${Math.floor(10 + Math.random() * 90)}`;

    // Insert into support_requests table using server admin client to bypass RLS restrictions
    ...
    if (insertErr) {
      ...
      // Fallback try without explicit request_number in case schema uses auto-generated sequence
    ...
    const request_number = inserted?.request_number || generatedReqNum;
    ...
    try {
      ...
      if (adminProfiles && adminProfiles.length > 0) {
        const notificationsToInsert = adminProfiles.map((admin) => ({
          ...
          message: `${request_number} from ${name} (${user_type}): ${subject}`,
