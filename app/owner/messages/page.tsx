...
export default function OwnerMessagesPage() {
    ...
    async function loadOwnerContacts() {
      try {
        ...
        if (res.contacts && res.contacts.length > 0) {
          ...
          finalContacts = [...finalContacts, ...contractors];
  ...
  const filteredContacts = contacts.filter((c) => {
    ...
    const companyMatch = c.contractor?.company_name?.toLowerCase().includes(query);
    const emailMatch = c.email?.toLowerCase().includes(query);
    const matchesSearch = !searchQuery || nameMatch || companyMatch || emailMatch;
          ...
          <div className="p-4 border-b space-y-3 bg-muted/20">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border bg-background pl-10 pr-3.5 py-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-orange-500/50"
              />
            </div>
            ...
            <div className="grid grid-cols-3 gap-1 bg-muted p-1 rounded-xl text-[11px] font-bold">
              <button
                ...
                className={`py-1.5 rounded-lg text-center transition-all ${
                  tabFilter === "ALL" ? "bg-orange-600 text-white shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                All
              </button>
              ...
              <button
                onClick={() => setTabFilter("contractor")}
                className={`py-1.5 rounded-lg text-center transition-all ${
                  tabFilter === "contractor" ? "bg-orange-600 text-white shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                Contractors
              </button>
            ...
            {loadingContacts ? (
              ...
              filteredContacts.map((c) => {
                ...
                const displayName = capitalize(
                  isAdmin ? "NIRMAN Admin Support" : c.contractor?.company_name || c.full_name
                );
          ...
          {selectedContact ? (
            <>
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`flex h-10 w-10 items-center justify-center rounded-2xl font-bold text-xs text-white shadow-sm ${
                      selectedContact.role === "admin" ? "bg-amber-600" : "bg-orange-600"
                    }`}
                  >
                    {selectedContact.role === "admin" ? (
                      <Shield className="h-5 w-5" />
                    ) : (
                      capitalize(selectedContact.contractor?.company_name || selectedContact.full_name)
                        .substring(0, 2)
                        .toUpperCase()
                    )}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold text-sm text-foreground">
                        {capitalize(
                          selectedContact.role === "admin"
                            ? "NIRMAN Platform Admin Support"
                            : selectedContact.contractor?.company_name || selectedContact.full_name
                        )}
                      </h3>
                      <span
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {selectedContact.role === "admin"
                        ...
                        : "Verified Contractor • Direct Project Chat"}
                    </p>
                  </div>
                </div>
              ...
              <form onSubmit={handleSendMessage} className="p-4 border-t flex flex-col gap-2 bg-card shrink-0">
                {selectedImage && (
                  <div className="flex items-center gap-2 rounded-xl bg-orange-500/10 border border-orange-500/30 p-2 text-xs text-orange-600 dark:text-orange-400 w-fit">
                    ...
                    <button
                      ...
                      className="p-1 rounded-full hover:bg-orange-500/20 text-rose-500"
                    >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-xl bg-orange-600 text-white shadow-md hover:bg-orange-700 disabled:opacity-50 shrink-0"
                  >
          ...
          <div className="relative max-w-4xl max-h-[90vh] w-full flex flex-col items-center bg-slate-900 border border-slate-800 rounded-3xl p-4 sm:p-6 shadow-2xl overflow-hidden">
            ...
            <div className="w-full flex items-center justify-between pb-3 border-b border-slate-800 text-white mb-4">
              <span className="text-xs sm:text-sm font-extrabold flex items-center gap-2">
                <ImageIcon className="h-4 w-4 text-orange-400" /> Image Attachment Preview
              </span>
              ...
              <div className="flex items-center gap-2">
                <a
                  ...
                  className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-extrabold text-xs transition-colors shadow-md"
                >
