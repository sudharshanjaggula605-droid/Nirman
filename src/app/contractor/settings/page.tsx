...
import { Key, Bell, Save, Globe, Check, AlertCircle, Loader2, Lock, Mail, User, Phone, Building2, Info, ShieldCheck } from "lucide-react";
import { useLanguage } from "@/frontend/lib/i18n/language-context";
import { changeUserPasswordAction } from "@/backend/actions/auth";
import { updateContractorProfileAction } from "@/backend/actions/user-settings";
...
export default function ContractorSettingsPage() {
  ...
  // Account & Email Credentials State
    ...
    async function loadAccountData() {
      try {
        ...
        if (user) {
          ...
          const email = prof?.email || user.email || cont?.email || "";
          setAccountEmail(email);
          setInitialEmail(email);
          setCompanyName(cont?.company_name || prof?.full_name || "");
          setContactPerson(cont?.contact_person || prof?.full_name || "");
          setAccountPhone(cont?.phone || prof?.phone || user.user_metadata?.phone || "");
          setAccountCity(cont?.city || prof?.city || "");
        }
      } catch (err) {
        console.error("Error loading contractor account in settings:", err);
      } finally {
  ...
  const handleUpdateAccount = async (e: React.FormEvent) => {
    ...
    if (!companyName.trim() && !contactPerson.trim()) {
      setAccountError("Company name or contact person is required.");
      return;
    }
    ...
    try {
      const res = await updateContractorProfileAction({
        ...
        contact_person: contactPerson.trim() || companyName.trim(),
      ...
      <div className="rounded-2xl border bg-card p-6 space-y-8 shadow-sm text-xs">
        ...
        <div className="space-y-4 pt-2 border-t">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Update your registered company credentials and login email address. If you change your email, your previous email will be invalidated immediately and you will only be able to sign in using your new email address.
          </p>
          ...
          <form onSubmit={handleUpdateAccount} className="space-y-4 max-w-xl">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="font-bold text-foreground text-xs">Company Name *</label>
              ...
              <div className="space-y-1">
                <label className="font-bold text-foreground text-xs">Contact Person *</label>
                <div className="relative">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="contractor@nirman.com"
                    className="w-full rounded-xl border border-border bg-background pl-9 pr-3.5 py-2.5 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-orange-500/50"
                  />
                </div>
              </div>
        ...
        <div className="space-y-4 pt-2 border-t">
          ...
          <form onSubmit={handleChangePassword} className="space-y-4 max-w-xl">
            {/* Current Password */}
            <div className="space-y-1">
              <label htmlFor="current_password" className="font-bold text-foreground flex items-center justify-between">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {/* New Password */}
              <div className="space-y-1">
                <label htmlFor="new_password" className="font-bold text-foreground flex items-center justify-between">
              ...
              <div className="space-y-1">
                <label htmlFor="confirm_password" className="font-bold text-foreground flex items-center justify-between">
