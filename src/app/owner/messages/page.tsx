...
export default function OwnerMessagesPage() {
  ...
  const [tabFilter, setTabFilter] = useState<"ALL" | "admin" | "contractor">("ALL");
    ...
    async function loadContacts() {
      try {
        ...
        if (res.contacts && res.contacts.length > 0) {
          ...
          const contractors = others.filter((p: any) => p.role === "contractor");
          let final: any[] = [];
          if (adminProf) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (result?.error) console.error("Failed to send:", result.error);
    setSending(false);
  };
  ...
  const extractImage = (msg: any) => {
    ...
    if (text.startsWith("data:image/") || (text.startsWith("http") && (text.includes(".png") || text.includes(".jpg")))) return text.trim();
    return null;
  };
  ...
  const cleanText = (msg: any) => {
    ...
    if (text.startsWith("data:image/") || (text.startsWith("http") && text.includes(".jpg"))) return "";
    return text;
  };
      ...
      <div className="p-3 border-b space-y-2 bg-muted/20">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border bg-background pl-9 pr-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-orange-500/50"
          />
        </div>
        <div className="grid grid-cols-3 gap-1 bg-muted p-1 rounded-xl text-[11px] font-bold">
          {(["ALL", "admin", "contractor"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setTabFilter(f)}
              className={`py-1.5 rounded-lg text-center transition-all capitalize ${
                tabFilter === f
                  ? f === "contractor" ? "bg-emerald-600 text-white shadow-sm" : "bg-orange-600 text-white shadow-sm"
                  : "text-muted-foreground hover:text-foreground"
              }`}
            >
              {f === "ALL" ? "All" : f === "admin" ? "Admin" : "Workers"}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto divide-y">
        {loadingContacts ? (
          ...
          filteredContacts.map((c) => {
            ...
            const isContractor = c.role === "contractor";
            const name = cap(isAdmin ? "NIRMAN Admin Support" : c.full_name);
            return (
              <button
                key={c.id}
                onClick={() => handleSelectContact(c)}
                className={`w-full p-3 flex items-center gap-3 text-left transition-all hover:bg-accent ${isSelected ? "bg-orange-500/10 border-l-4 border-orange-600" : ""}`}
              >
                <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl font-bold text-xs text-white shadow-sm ${isAdmin ? "bg-orange-600" : isContractor ? "bg-emerald-600" : "bg-slate-600"}`}>
                  {isAdmin ? <Shield className="h-4 w-4" /> : isContractor ? <HardHat className="h-4 w-4" /> : name.substring(0, 2).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1">
                    <p className="font-semibold text-xs text-foreground truncate">{name}</p>
                    <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold uppercase shrink-0 ${isAdmin ? "bg-orange-500/10 text-orange-600 border border-orange-500/20" : "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20"}`}>
  ...
  const ChatPanel = selectedContact ? (
    <div className="flex flex-col h-full">
      <div className="p-3 border-b bg-muted/20 flex items-center gap-3 shrink-0">
        ...
        <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl font-bold text-xs text-white shadow-sm ${selectedContact.role === "admin" ? "bg-orange-600" : "bg-emerald-600"}`}>
          {selectedContact.role === "admin" ? <Shield className="h-4 w-4" /> : <HardHat className="h-4 w-4" />}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold uppercase shrink-0 ${selectedContact.role === "admin" ? "bg-orange-500/10 text-orange-600 border border-orange-500/20" : "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20"}`}>
              {selectedContact.role === "admin" ? "admin" : "contractor"}
            </span>
          </div>
          <p className="text-[11px] text-muted-foreground truncate">
            {selectedContact.role === "admin" ? "Platform Administration & Support" : "Contractor • Project Chat"}
          </p>
        </div>
          ...
          messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex flex-col ${isSelf ? "items-end" : "items-start"}`}>
                <div className={`max-w-[85%] sm:max-w-lg rounded-2xl p-3 text-xs space-y-1.5 shadow-sm ${isSelf ? "bg-orange-600 text-white rounded-br-none" : "bg-muted text-foreground border rounded-bl-none"}`}>
      ...
      <form onSubmit={handleSendMessage} className="p-3 border-t flex flex-col gap-2 bg-card shrink-0">
        {selectedImage && (
          <div className="flex items-center gap-2 rounded-xl bg-orange-500/10 border border-orange-500/30 p-2 text-xs text-orange-600 w-fit">
            <ImageIcon className="h-4 w-4 shrink-0" />
            <span className="font-bold truncate max-w-[160px]">{selectedImageName || "Photo"}</span>
            <button type="button" onClick={() => { setSelectedImage(null); setSelectedImageName(null); }} className="p-1 rounded-full hover:bg-orange-500/20 text-rose-500">
              <X className="h-3 w-3" />
            </button>
          </div>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 rounded-xl border bg-background px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-orange-500/50"
          />
          <button type="submit" disabled={sending || (!inputContent.trim() && !selectedImage)} className="p-2 rounded-xl bg-orange-600 text-white shadow-md hover:bg-orange-700 disabled:opacity-50 shrink-0">
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="flex-1 overflow-hidden">
        ...
        <div className="hidden md:grid md:grid-cols-12 h-full">
          <div className="md:col-span-4 border-r overflow-hidden flex flex-col">{ContactsPanel}</div>
          <div className="md:col-span-8 overflow-hidden flex flex-col">{ChatPanel}</div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="relative max-w-4xl max-h-[90vh] w-full flex flex-col items-center bg-slate-900 border border-slate-800 rounded-3xl p-4 shadow-2xl overflow-hidden">
            <div className="w-full flex items-center justify-between pb-3 border-b border-slate-800 text-white mb-4">
              <span className="text-xs font-extrabold flex items-center gap-2"><ImageIcon className="h-4 w-4 text-orange-400" /> Image Preview</span>
              <div className="flex items-center gap-2">
                <a href={previewModalImg} download="nirman_attachment.png" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs transition-colors">
